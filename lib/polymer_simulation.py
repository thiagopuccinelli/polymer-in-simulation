...
class PolymerSimulation:
    ...
    def __initialize_lammps_script(self):
        with open("lammps_"+str(self.filename)+".in", "w") as lmpScript:
            ...
            lmpScript.write("fix myPOLYmsd all ave/time 100 1 100 c_myChunk2[*] file "+str(self.filename)+"_msd_polymers_each.dat mode vector\n")
            lmpScript.write("compute polymer_msd polymer msd \n")
            lmpScript.write("fix polymer_msd polymer ave/time 2 6 100 c_polymer_msd[4] file "+str(self.filename)+"_msd_polymer.dat\n")
